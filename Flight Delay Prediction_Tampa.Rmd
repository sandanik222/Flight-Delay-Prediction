---
title: "Assignment 9"
author: "Hasmik Grigoryan, Hanyu Xiao, Rose Porta, Sandani Kumanayake"
date: "2023-10-05"
output: pdf_document
---
...
flight.data <- read.csv("TampaFlights2022.csv",
...
# model
model <- model.matrix(delay ~ (day + depart + duration + month)^2 + carrier 
                      + poly(depart, degree=3,raw=TRUE)[,2:3]+ poly(duration,degree=3,raw=TRUE)[,2:3],data=train.batch)[,-1]


glmmod <- glmnet(model,y=as.factor(train.batch$delay),alpha=1,family='binomial')

cv.glmmod <- cv.glmnet(model,y=train.batch$delay,alpha=1)
plot(cv.glmmod)

best_lambda <- cv.glmmod$lambda.min

model.results <- coef(glmmod,s=best_lambda)

model.results

model.testdata <- model.matrix(delay ~ (day + depart + duration + month)^2 + carrier + poly(depart, degree=3,raw=TRUE)[,2:3]+ poly(duration,degree=3,raw=TRUE)[,2:3],data=test.batch)[,-1]


glm.predict.lasso <- as.vector(predict(glmmod, s=best_lambda,model.testdata,type="response"))

glm.predict.lasso.outcome <- factor(as.numeric(glm.predict.lasso > 0.5))

confusion.outcome.lasso <- confusionMatrix(glm.predict.lasso.outcome,factor(test.batch$delay))

print(confusion.outcome.lasso)

model_columns <- colnames(model)

model.testdata

save(glmmod, file="lasso_model.Rdata")
save(model_columns, file="model_columns.Rdata")
...
# main function
# inputs: month, day, carrier, depart, duration
# output: probability of delay
predict.delay <- function(month, day, carrier, depart, duration){
  
  # best lambda returned by the trained model
  best_lambda<-1.341975e-05
  
  # load the model columns and the model saved previously
  load("lasso_model.RData") 
  load("model_columns.RData") 
  
  # make a dataframe with the columns from model_columns  
  df <- data.frame(matrix(ncol = 80, nrow = 0))
  colnames(df) <- model_columns
  
  # make the main effect variables
  day2 <- 1*(day==2)
  day3 <- 1*(day==3)
  day4 <- 1*(day==4)
  day5 <- 1*(day==5)
  ...
  # make the full interaction and polynomial row 
  ...
  prob <- as.vector(predict(glmmod, s=best_lambda, as.matrix(df[1,]),type="response"))
...
#Example call
# input: month, day, caarrier, depart, duration
predict.delay(7, 4, "AA", 722, 98)
```
...
The code is heavily commented as part of documentation. 
...
Below are some examples of calling the function. 

The probability of delay for a flight with the following parameters 
+ departs in July
+ on a Thursday
...
+ departs at 12.01 a.m.
+ lasts 200 minutes. 

Probability of delay:
```{r}
predict.delay(9, 3, "NK", 1, 200)
```
...
# model
model <- model.matrix(delay ~ (day + depart + duration + month)^2 
                      + carrier + poly(depart, degree=3,raw=TRUE)[,2:3]
                      + poly(duration,degree=3,raw=TRUE)[,2:3],data=train.batch)[,-1]
...
model.testdata <- model.matrix(delay ~ (day + depart + duration + month)^2 
                               + carrier + poly(depart, degree=3,raw=TRUE)[,2:3]
                               + poly(duration,degree=3,raw=TRUE)[,2:3],data=test.batch)[,-1]
